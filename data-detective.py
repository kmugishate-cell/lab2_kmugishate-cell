...
def safe_int(value, default=0):
    """
    Safely converts a value to an int. Returns `default` if the value
    is missing, empty, or not a valid number (protects against crashes
    on messy/corrupted data).
    """
    try:
...
def clean_data(tweets):
    ...
    - Removes any tweet with a missing/blank 'Text' field.
    - Replaces missing OR non-numeric 'Likes'/'Retweets' with 0.
    - Strips stray whitespace from 'Text'.
    Returns the cleaned list and prints how many rows were fixed/removed.
    """
    clean_tweets = []
    fixed_count = 0
    removed_count = 0
    ...
    for tweet in tweets:
        ...
        # Replace missing or non-numeric Likes with 0
        likes = tweet.get("Likes")
        if likes is None or str(likes).strip() == "" or not str(likes).strip().lstrip("-").isdigit():
            tweet["Likes"] = "0"
            row_was_fixed = True
 
        # Replace missing or non-numeric Retweets with 0
        retweets = tweet.get("Retweets")
        if retweets is None or str(retweets).strip() == "" or not str(retweets).strip().lstrip("-").isdigit():
            tweet["Retweets"] = "0"
            row_was_fixed = True
...
def find_viral_tweet(tweets):
    ...
    top_tweet = tweets[0]
    highest_likes = safe_int(top_tweet["Likes"])
 
    for tweet in tweets[1:]:
        current_likes = safe_int(tweet["Likes"])
...
def custom_sort_by_likes(tweets):
    """
    QUEST 3: Implement Selection Sort to sort the list by 'Likes' in
    descending order. NO .sort() or sorted() allowed!
    """
    # Work on a copy so we don't mutate the original list order unexpectedly
    ...
    for i in range(n):
        max_index = i
        max_likes = safe_int(sorted_tweets[max_index]["Likes"])
 
        for j in range(i + 1, n):
            current_likes = safe_int(sorted_tweets[j]["Likes"])
...
if __name__ == "__main__":
    ...
    # QUEST 3: Custom sort by Likes (descending), show Top 10
